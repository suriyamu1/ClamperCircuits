...
function incrementStep() {
    ...
    if(count<6 && design_verified == true) {
...
function change_Circuit_Graph_Explanation() {
    ...
    document.getElementById("diagram").innerHTML = '<img src="images/1_Pos_unbiased_clamper/Circuit_pics/Slide'+count+'.JPG" alt="">';

    //changing graph_image
    document.getElementById("output_graph").src="images/1_Pos_unbiased_clamper/Graph_pics/Slide"+count+".JPG";


    //changing explanation "text"
    if(design_verified==false) {
        document.getElementById("explanation").innerHTML = '<h2>Choose the components correctly.<br>Verify your design<br>Then press the "Next" button.</h2>';
    }
    else {
        if(count==1) {
        ...
        if(count==4) {
            document.getElementById("explanation").innerHTML = '<h2><span class="bolder_black">When first negative peak is reached, </span>the voltage across the diode is zero. This turns off the diode. Now the output voltage is the sum of input voltage and capacitor voltage. The capacitor now acts like a DC source, thus shifting the waveform above by a DC value of +peak voltage.</h2>';
        }
